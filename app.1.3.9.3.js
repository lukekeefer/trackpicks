...
async function refreshCanonicalApp(latestVersion){
  ...
  // Fetch the canonical root explicitly with cache-busting/no-store first.
  // This refreshes the document the installed PWA launches on cold start,
  // instead of only loading a one-off query-string URL.
  try{
    await fetch(`${canonicalUrl.toString()}?__tp_refresh=${Date.now()}`,{
      cache:'reload',
  ...
  // Land on the canonical app URL (not a temporary versioned query URL),
  // so subsequent home-screen launches reopen the same current document.
