const BUILD_VERSION = '20260924-07';
...
function toDbWager(w){ return {id:w.id,user_id:state.user.id,game_id:w.gameId,bet_type:w.betType,selection:w.selection,line:w.line,units:w.units,who:w.who,pick:w.pick,result:w.result||'Pending',market_spread:w.marketSpread,market_total:w.marketTotal,market_moneyline:w.marketMoneyline,updated_at:new Date().toISOString()}; }
...
function renderParlayLeg(leg,draft){
  ...
  const labelFor=(v)=>leg.betType==='Spread'?`${leg.selection} ${signed(v)}`:leg.betType==='Moneyline'?`${leg.selection} ${formatAmericanOdds(v)}`:`${leg.selection} ${v}`;
  const baseLabel=labelFor(base),finalLabel=labelFor(finalLine);
  return `<div class="parlay-leg"><div class="parlay-leg-copy"><strong>${draft.isTeaser&&leg.betType!=='Moneyline'?`${baseLabel} → ${finalLabel}`:baseLabel}</strong><span>${g?`${g.away} @ ${g.home}`:'Game unavailable'} · ${leg.betType}</span></div><button class="parlay-leg-remove" data-remove-parlay-leg="${leg.id}" aria-label="Remove leg">✕</button></div>`;
}
  ...
  return `<div class="overlay">
    <section class="sheet game-detail-sheet">
      ...
      <div class="game-meta-row">
        ...
        <div class="game-meta-item">
          <span class="game-meta-icon">${metaIcon('tv')}</span>
          <div><strong>${escapeAttr(g.tv||'TBD')}</strong></div>
        </div>
        <div class="game-meta-divider"></div>
      ...
      <div class="wager-editor">
        ...
        <div class="field-grid">
          <div class="field">
            <label>Actual line taken</label>
            <input id="lineInput" type="number" step="0.5" value="${actualLineValue}" placeholder="Optional — defaults to market">
          </div>
          <div class="field">
            <label>Units</label>
...
function bindDynamicSelections(){ document.querySelectorAll('[data-bet-kind][data-selection]').forEach(el=>el.onclick=()=>{if(el.disabled)return;tempKind=el.dataset.betKind;tempSelection=el.dataset.selection;document.querySelectorAll('[data-bet-kind][data-selection]').forEach(b=>b.classList.toggle('selected',b.dataset.betKind===tempKind&&b.dataset.selection===tempSelection));}); }
function bind(){
...
function addCurrentSelectionToParlay(){
  ...
  state.parlayDraft.legs.push({id:crypto.randomUUID(),gameId:g.id,betType:tempKind,selection:tempSelection,sourceLine:Number(line),odds:marketOddsFor(g,tempKind,tempSelection)});
  syncParlayEstimate();
...
async function saveCurrentWager(){
  if(state.saving)return;captureWagerDraft();const g=gameById(state.activeGameId),editing=state.editWagerId?state.wagers.find(w=>w.id===state.editWagerId):null,rawLine=String(tempLine??'').trim(),units=Number(tempUnits||1),who=(tempWho||document.getElementById('whoInput').value||state.displayName);if(!tempKind||!tempSelection){alert('Choose a wager first.');return;}const marketLine=marketLineFor(g,tempKind,tempSelection);if(marketLine==null){alert('The selected DraftKings market is unavailable for this game.');return;}const finalLine=rawLine===''?marketLine:Number(rawLine);if(Number.isNaN(finalLine)){alert('Enter a valid line/odds value, or leave the field blank to use the market.');return;}if(!Number.isFinite(units)||units<=0){alert('Enter a valid unit amount greater than 0.');return;}const pick=tempKind==='Spread'?`${tempSelection} ${signed(finalLine)}`:tempKind==='Moneyline'?`${tempSelection} ${formatAmericanOdds(finalLine)}`:`${tempSelection} ${finalLine}`;const obj={id:state.editWagerId||crypto.randomUUID(),gameId:g.id,betType:tempKind,selection:tempSelection,line:finalLine,units,who,pick,result:editing?.result||'Pending',marketSpread:g.spread,marketTotal:g.total,marketMoneyline:tempKind==='Moneyline'?marketLine:null};state.saving=true;render();const {error}=await state.sb.from('wagers').upsert(toDbWager(obj));if(error){state.saving=false;alert(`Could not save pick: ${error.message}`);render();return;}if(editing){state.wagers[state.wagers.findIndex(w=>w.id===obj.id)]=obj;}else state.wagers.push(obj);render();setTimeout(()=>{state.activeGameId=null;state.editWagerId=null;tempWho=null;resetWagerDraft();state.saving=false;render();},2200);
}
