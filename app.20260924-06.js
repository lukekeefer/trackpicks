const BUILD_VERSION = '20260924-06';
...
function renderChoiceArea(g,kind,selection){
  ...
  return `<div class="bet-type-grid direct-wager-grid">
    <button class="choice-btn ${kind==='Spread'&&selection===g.away?'selected':''}" data-bet-kind="Spread" data-selection="${escapeAttr(g.away)}" ${spreadAvailable?'':'disabled'}>${g.away} ${awayLine}</button>
    <button class="choice-btn ${kind==='Spread'&&selection===g.home?'selected':''}" data-bet-kind="Spread" data-selection="${escapeAttr(g.home)}" ${spreadAvailable?'':'disabled'}>${g.home} ${homeLine}</button>
    <button class="choice-btn ${kind==='Total'&&selection==='Over'?'selected':''}" data-bet-kind="Total" data-selection="Over" ${totalAvailable?'':'disabled'}>Over ${totalLine}</button>
    <button class="choice-btn ${kind==='Total'&&selection==='Under'?'selected':''}" data-bet-kind="Total" data-selection="Under" ${totalAvailable?'':'disabled'}>Under ${totalLine}</button>
  </div>`;
}
    ...
    <section class="sheet game-detail-sheet">
      ...
      <section class="matchup-hero">
        ...
        <div class="matchup-center">
          ...
          <div class="matchup-date">${k.date}</div>
          <div class="matchup-time">${k.time}</div>
        </div>
      ...
      <div class="game-info-strip">
        <div class="game-info-item">
          <span>TV</span>
          <strong>${escapeAttr(g.tv||'TBD')}</strong>
        </div>
        <div class="game-info-item location-info">
          <span>Location</span>
          <strong>${escapeAttr(venue)}</strong>
        </div>
      </div>
      ...
      <div class="wager-editor">
        <div class="section-title">${editing?'Edit wager':'Add wager'}</div>
        ...
        <button type="button" class="caution-toggle ${isCautioned(g.id)?'active':''}" data-toggle-caution="${g.id}">${isCautioned(g.id)?'⚠️ Caution Marked':'⚠️ Mark Caution'}</button>
