const BUILD_VERSION = '20260923-12';
...
function bind(){
  ...
  document.querySelectorAll('[data-remove-parlay-leg]').forEach(el=>el.onclick=()=>{state.parlayDraft.legs=state.parlayDraft.legs.filter(l=>l.id!==el.dataset.removeParlayLeg);render();});
  document.querySelectorAll('[data-clear-parlay]').forEach(el=>el.onclick=()=>{resetParlayDraft();render();});
  document.querySelectorAll('[data-toggle-teaser]').forEach(el=>el.onclick=()=>{captureParlayDraftInputs();state.parlayDraft.isTeaser=!state.parlayDraft.isTeaser;render();});
...
function addCurrentSelectionToParlay(){
  ...
  if(!tempSelection)tempSelection=tempKind==='Spread'?g.spreadTeam:'Over';
