...
function tvNetworkLogoKey(tv){
  ...
  if(n==='BTN' || n.includes('BIG TEN NETWORK'))return 'btn';
  return null;
}
function renderTvNetworkLogo(tv){
  const key=tvNetworkLogoKey(tv);
  if(!key)return `<span class="game-meta-icon">${metaIcon('tv')}</span>`;
  const labels={
    espn:'ESPN',espn2:'ESPN2',espnu:'ESPNU',espnplus:'ESPN+',
    abc:'abc',fox:'FOX',fs1:'FS1',fs2:'FS2',
    cbs:'CBS',cbssn:'CBS SPORTS',nbc:'NBC',peacock:'PEACOCK',
    cw:'THE CW',accn:'ACCN',secn:'SECN',btn:'B1G'
  };
  return `<span class="tv-network-logo tv-network-${key}" aria-label="${escapeAttr(tv)}">${labels[key]}</span>`;
}
  ...
  return `<div class="overlay">
    <section class="sheet game-detail-sheet">
      ...
      <div class="game-meta-row">
        ...
        <div class="game-meta-item tv-meta-item">
