const BUILD_VERSION = '20260923-9';
...
function filteredSlateGames(){
  ...
  if(state.slateDivision==='FCS'){
    return games.filter(g=>g.sourceSportKey==='americanfootball_ncaaf_fcs');
  }
  return games.filter(g=>g.sourceSportKey==='americanfootball_ncaaf')
              .filter(g=>gameMatchesConference(g,state.slateConference));
}
function renderSlateFilters(){
  const division=`<div class="slate-filter-primary">\n    <button class="slate-filter-btn primary-filter ${state.slateDivision==='FBS'?'active':''}" data-slate-division="FBS">FBS</button>\n    <button class="slate-filter-btn primary-filter ${state.slateDivision==='FCS'?'active':''}" data-slate-division="FCS">FCS</button>\n  </div>`;
